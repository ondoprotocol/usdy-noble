...
if ! [ -f .aura/data/priv_validator_state.json ]; then
  ...
  SOURCE_OWNER=$(aurad keys add source-owner --home .aura --keyring-backend test --output json | jq .address)
  aurad add-genesis-account source-owner 10000000uusdc --home .aura --keyring-backend test
